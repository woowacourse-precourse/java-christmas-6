...
public class PromotionChecker {
    ...
    private static final DayOfWeek[] WEEKDAYS = {DayOfWeek.SUNDAY, DayOfWeek.MONDAY,
        DayOfWeek.TUESDAY, DayOfWeek.WEDNESDAY, DayOfWeek.TUESDAY};
    ...
    private void checkPresentPromotion(Reservation reservation,
        Map<Promotion, Integer> promotionApplied) {
    ...
    private void checkSpecialPromotionAppliable(Reservation reservation,
        Map<Promotion, Integer> promotionApplied) {
        if (reservation.checkReservedDate().getDayOfMonth() == 25
            || reservation.checkReservedDate().getDayOfWeek() == DayOfWeek.SUNDAY) {
            promotionApplied.put(Promotion.SPECIAL_PROMOTION,
                Promotion.SPECIAL_PROMOTION.checkPromotionDefaultBenefit());
        }
    }

    private void checkWeekendPromotionAppliable(Reservation reservation,
        Map<Promotion, Integer> promotionApplied) {
        if (Arrays.stream(WEEKENDS).toList().contains(
            reservation.checkReservedDate().getDayOfWeek()
        )) {
            List<SimpleEntry<Menu, Integer>> menuPromotionApplied = reservation.checkMenuByCategory(
                WEEKEND_PROMOTION_CATEGORY);

            int matchedMenuByPromotionCategory = menuPromotionApplied.stream()
                .mapToInt(SimpleEntry::getValue).sum();
            int totalBenefit = matchedMenuByPromotionCategory
                * Promotion.WEEKEND_PROMOTION.checkPromotionDefaultBenefit();

            promotionApplied.put(Promotion.WEEKEND_PROMOTION, totalBenefit);
        }
    }

    private void checkWeekdayPromotionAppliable(Reservation reservation,
        Map<Promotion, Integer> promotionApplied) {
        if (Arrays.stream(WEEKDAYS).toList()
            .contains(reservation.checkReservedDate().getDayOfWeek())) {
            List<SimpleEntry<Menu, Integer>> menuPromotionApplied = reservation.checkMenuByCategory(
                WEEKDAY_PROMOTION_CATEGORY);

            int matchedMenuByPromotionCategory = menuPromotionApplied.stream()
                .mapToInt(SimpleEntry::getValue).sum();
            int totalBenefit = matchedMenuByPromotionCategory
                * Promotion.WEEKDAY_PROMOTION.checkPromotionDefaultBenefit();

            promotionApplied.put(Promotion.WEEKDAY_PROMOTION, totalBenefit);
        }
    }

    private void checkChristmasPromotionAppliable(Reservation reservation,
        Map<Promotion, Integer> promotionApplied) {
        if (reservation.isIncludedPromotionPeriod(Promotion.CHRISTMAS_DDAY_PROMOTION)) {
            ...
            int christmasPromotionBenefit =
                Promotion.CHRISTMAS_DDAY_PROMOTION.checkPromotionDefaultBenefit() + interval * 100;
