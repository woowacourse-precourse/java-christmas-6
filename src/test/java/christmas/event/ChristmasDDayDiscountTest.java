...
class ChristmasDDayDiscountTest {
    ...
    @DisplayName("날짜를 입력했을 때, 이벤트 날짜에 해당되면 할인금액을 반환한다.")
    @Test
    void whenEventActivate() {
        Integer discountAmount = christmasDDayDiscount.executeEvent(LocalDate.of(2023, Month.DECEMBER, 7));

        assertThat(discountAmount).isEqualTo(1600);
    }
